    ...
    <div className="h-screen bg-cyan-50 overflow-hidden relative">
			...
			<div className="container h-screen flex flex-wrap items-center justify-center px-4 sm:px-8 lg:px-48 mx-auto">
				<div className="w-full lg:w-2/4 h-[40vh] lg:h-[90vh] flex items-end bg-login-bg-img bg-cover bg-center rounded-lg p-5 lg:p-10 z-50 mb-4 lg:mb-0">
					<div>
						<h4 className="text-3xl lg:text-5xl text-white font-semibold leading-tight lg:leading-[58px]">{t("login.captureYour_3")}
              <br />{t("login.journeys_3")}
            </h4>
						<p className="text-sm lg:text-[15px] text-white leading-5 lg:leading-6 mt-2 lg:mt-4 pr-3 lg:pr-7">{t("login.recordYourTravelExperiencesAnd_3")}
				...
				<div className="w-full lg:w-2/4 bg-white rounded-lg lg:rounded-r-lg relative p-8 lg:p-16 shadow-lg shadow-cyan-200/20">
					<form onSubmit={handleLogin}>
						<h4 className="text-xl lg:text-2xl font-semibold mb-5 lg:mb-7">{t("login.login_5")}

            </h4>

						<input
              type="text"
              placeholder={t("login.email_3")}
              className="input-box"
              value={email}
              onChange={({ target }) => {
              ...
              onChange={({ target }) => {
                setPassword(target.value);
						...
						<button type="submit" className="btn-primary">{t("login.login_6")}

            </button>

						<p className="text-xs text-slate-500 text-center my-4">{t("login.or_3")}</p>
