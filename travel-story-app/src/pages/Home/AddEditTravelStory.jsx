  ...
  return (
    <div className="relative w-full max-w-4xl mx-auto p-4">
          ...
          onClick={onClose}>
            ...
            <button
              ...
              onClick={handleAddOrUpdateClick}>
								...
								<MdAdd className="text-lg" />{t("add-edit-travel-story.addStory_4")}
            </button> :
            ...
            <>
                ...
                onClick={handleAddOrUpdateClick}>
									...
									<MdUpdate className="text-lg" />{t("add-edit-travel-story.updateStory_4")}
              </button>
								<button
                className="btn-small btn-delete w-full sm:w-auto flex items-center justify-center gap-2"
                onClick={onClose}>

									<MdDeleteOutline className="text-lg" />{t("add-edit-travel-story.delete_2")}
              </button>
							</>
            }
          ...
          <p className="text-red-500 text-xs pt-2 text-left sm:text-right">
			...
			<div className="space-y-6">
				<div className="flex flex-col gap-2">
					<label className="input-label">{t("add-edit-travel-story.title_2")}</label>
					<input
            type="text"
            className="text-xl sm:text-2xl text-slate-950 outline-none w-full p-2 bg-slate-50 rounded"
            placeholder={t("add-edit-travel-story.aDayAtGreatWall_2")}
            value={title}
          ...
          handleDeleteImg={handleDeleteStoryImg} />
				...
				<div className="flex flex-col gap-2">
					<label className="input-label">{t("add-edit-travel-story.story_2")}</label>
					<textarea
            className="text-sm text-slate-950 outline-none bg-slate-50 p-4 rounded min-h-[200px] w-full"
            placeholder={t("add-edit-travel-story.yourStory_2")}
            rows={10}
            value={story}
            onChange={({ target }) => setStory(target.value)} />
				...
				<div>
					<label className="input-label block mb-2">{t("add-edit-travel-story.visitedLocations_2")}</label>
